...
import { LayoutDashboard, MessageCircle, Phone, Settings, Activity, ShieldCheck, BookText, Info } from 'lucide-react';
  ...
  return (
    <div className="w-64 glass-sidebar flex flex-col h-full sticky top-0 z-50">
      <div className="p-8 flex items-center gap-3">
        ...
        <span className="font-bold text-white text-xl tracking-tight">Sonia</span>
      </div>
      ...
      <nav className="flex-1 px-4 py-6 space-y-2">
        {menuItems.map((item) => (
          ...
          >
            ...
            <item.icon className={`w-5 h-5 transition-colors ${location.pathname === item.path ? 'text-indigo-400' : 'text-slate-500 group-hover:text-slate-300'}`} />
            {item.label}
          </Link>
        ))}
      </nav>
