...
import { Brain, Heart, Zap, Sparkles, TrendingUp, FileText, Loader2, Activity, History, LogIn, UserPlus } from 'lucide-react';
...
const Dashboard: React.FC<DashboardProps> = ({ messages, userContext, onLogout }) => {
  ...
  const handleGenerateReport = async () => {
    ...
    try {
      const report = await gemini.generateSummaryReport([], messages, userContext);
  ...
  const chartData = useMemo(() => {
    ...
    if (data.length === 0) {
      return Array.from({ length: 5 }).map((_, i) => ({ time: `---`, intensity: 0 }));
    }
    return data;
  }, [messages]);
  ...
  const COLORS = {
    Happy: '#10b981', Sad: '#3b82f6', Stress: '#f59e0b', Anxiety: '#8b5cf6',
    Anger: '#ef4444', Burnout: '#64748b', Neutral: '#94a3b8', Excited: '#f43f5e'
  };
    ...
    <div className="p-6 lg:p-12 space-y-10 max-w-7xl mx-auto animate-in fade-in duration-700">
      <header className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-8">
        <div className="space-y-2">
          ...
          <p className="text-slate-400 font-medium">Real-time visualization of your emotional architecture.</p>
        </div>
        
        <div className="flex flex-wrap items-center gap-4">
          {/* Requested Login/Signup Buttons */}
          ...
          <button 
            ...
            className="px-8 py-3.5 bg-white/5 border border-white/10 hover:border-indigo-500/30 rounded-2xl text-xs font-black text-indigo-400 uppercase tracking-widest flex items-center gap-2 transition-all disabled:opacity-30"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="glass-card p-8 rounded-[2rem] flex items-center gap-6 group hover:border-rose-500/20">
          ...
          <div>
            ...
            <h2 className="text-3xl font-black text-white">{reportData?.stabilityScore || '84'}%</h2>
          </div>
        </div>
        <div className="glass-card p-8 rounded-[2rem] flex items-center gap-6 group hover:border-indigo-500/20">
          ...
          <div>
            <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Active Chat Nodes</p>
            <h2 className="text-3xl font-black text-white">{messages.filter(m => m.emotion).length}</h2>
          </div>
        </div>
        <div className="glass-card p-8 rounded-[2rem] flex items-center gap-6 group hover:border-emerald-500/20">
          ...
          <div>
            <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Historical Entries</p>
            <h2 className="text-3xl font-black text-white">{messages.length}</h2>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-8 glass-card p-10 rounded-[2.5rem] h-[450px]">
          <h3 className="text-xs font-black text-white uppercase tracking-widest mb-10 flex items-center gap-2">
            <TrendingUp className="text-indigo-400 w-4 h-4" /> Emotional Resonance Trend
          </h3>
          <div className="h-[300px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={chartData}>
                ...
                />
                <Area type="monotone" dataKey="intensity" stroke="#6366f1" strokeWidth={3} fill="url(#intensityGrad)" animationDuration={2000} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="lg:col-span-4 glass-card p-10 rounded-[2.5rem]">
          <h3 className="text-xs font-black text-white uppercase tracking-widest mb-8">Detected Patterns</h3>
          <div className="space-y-6">
            ...
            {distribution.length === 0 && (
              <div className="flex flex-col items-center justify-center py-20 text-center space-y-4 opacity-30">
                ...
                <p className="text-xs italic font-medium">Insufficient neural data.</p>
              </div>
            )}
          </div>
        </div>
          ...
          <div className="flex flex-col md:flex-row gap-10">
            <div className="flex-1 space-y-6">
              <div className="flex items-center gap-2 px-3 py-1 bg-indigo-500/10 rounded-full w-fit border border-indigo-500/20">
                ...
                <span className="text-[10px] font-black text-indigo-400 uppercase tracking-widest">AI Report Synthesis</span>
              </div>
              <h3 className="text-3xl font-black text-white">Longitudinal Trajectory</h3>
              <p className="text-lg text-slate-300 font-medium italic leading-relaxed">"{reportData.summary}"</p>
              <div className="flex flex-wrap gap-3">
                {reportData.keyThemes.map((theme: string, i: number) => (
                  <span key={i} className="px-4 py-2 bg-white/5 border border-white/5 rounded-xl text-[10px] font-black text-slate-300 uppercase tracking-widest">
                    {theme}
                  </span>
                ))}
              </div>
            ...
            <div className="w-full md:w-80 bg-indigo-500/10 p-10 rounded-[3rem] border border-indigo-500/20 flex flex-col justify-center">
              <h4 className="text-[10px] font-black text-indigo-400 uppercase tracking-[0.3em] mb-4">Neural Protocol</h4>
              <p className="text-sm font-bold text-white leading-relaxed italic">"{reportData.recommendation}"</p>
            </div>
          </div>
        </div>
      )}
